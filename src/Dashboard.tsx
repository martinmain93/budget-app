...
function AiSettingsPanel({
  ...
  function handleProviderChange(provider: AiProvider) {
    ...
    setEditModel(DEFAULT_MODELS[provider]);
  }

  function handleSave() {
    const keyToSave = editApiKey.trim() || aiSettings?.apiKey || "";
    if (!keyToSave) return;
  ...
  function handleRemoveKey() {
    void onUpdateAiSettings(undefined);
    setEditApiKey("");
    setShowKey(false);
  }
        ...
        {hasSavedKey && (
          <span
            style={{
              ...
              height: 8,
              borderRadius: "50%",
              backgroundColor: aiSettings?.enabled ? "#4caf50" : "#bbb",
              marginLeft: 6,
            }}
          />
        )}
      ...
      {expanded && (
        <div className="stack" style={{ marginTop: "0.5rem", gap: "0.5rem" }}>
          ...
          <label style={{ fontSize: "0.8rem", fontWeight: 600 }}>
            ...
            >
              {(Object.keys(PROVIDER_LABELS) as AiProvider[]).map((prov) => (
          ...
          <label style={{ fontSize: "0.8rem", fontWeight: 600 }}>
            ...
            <input
              ...
              placeholder={DEFAULT_MODELS[editProvider]}
          ...
          {hasSavedKey && !showKey ? (
            <div className="row" style={{ gap: "0.5rem", alignItems: "center" }}>
              ...
              <button type="button" className="ghost" style={{ fontSize: "0.75rem" }} onClick={() => setShowKey(true)}>
                Change
              </button>
            </div>
          ) : (
            <input
              ...
              placeholder={`Paste your ${PROVIDER_LABELS[editProvider]} API key`}
              autoComplete="off"
              style={{ width: "100%" }}
            />
          )}
          ...
          <div className="row" style={{ gap: "0.5rem", marginTop: "0.25rem" }}>
            <button type="button" className="primary" onClick={handleSave} disabled={!editApiKey.trim() && !hasSavedKey}>
              Save
            </button>
            {hasSavedKey && (
              <>
                <button type="button" className={aiSettings?.enabled ? "ghost" : "primary"} onClick={handleToggleEnabled}>
                  {aiSettings?.enabled ? "Disable" : "Enable"}
                </button>
                <button type="button" className="ghost danger" onClick={handleRemoveKey}>
                  Remove key
                </button>
              </>
            )}
          ...
          <small style={{ color: "var(--muted, #888)", lineHeight: 1.3 }}>
            Your API key is encrypted and stored in your vault. It is sent directly
            to {PROVIDER_LABELS[editProvider]} over HTTPS{editProvider === "anthropic" ? " via a stateless proxy (required for browser CORS)" : ""} and never stored on our servers.
          </small>
        </div>
      )}
    </div>
